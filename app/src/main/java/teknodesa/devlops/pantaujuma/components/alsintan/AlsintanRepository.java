...
public class AlsintanRepository implements AlsintanContract.Repository<AlsintanRealm> {
    @Inject
    Realm realm;

    private AlsintanContract.Controller mController;

    public AlsintanRepository(AlsintanContract.Controller mController, @NonNull AppComponent appComponent) {
    ...
    // obtain the results of a query
        final RealmResults<AlsintanRealm> results = realm.where(AlsintanRealm.class).equalTo("hashId", idItem).findAll();
    ...
    public void setItemDeleted(String idItem) {
        ...
        AlsintanRealm deletedItem = realm.where(AlsintanRealm.class).equalTo("hashId", idItem).findFirst();
    }
}
